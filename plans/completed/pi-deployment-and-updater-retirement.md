...
| 2b — GBot-Docs deployment-docs rewrite | **done 2026-07-26** — [GBot-Docs#5](https://github.com/cgoulart35/GBot-Docs/pull/5) (`3cc2886`). Its README's `# GBot Development` half was a pre-modernization copy of GBot's, 126 lines adrift: still documented the updater, and had **no** deployment section. Synced lines 529–end from GBot's canonical README (updater refs out; Deployment / One-time Pi bring-up / Rollback in; plus Coverage, CI, PR auto-review, and the current `scripts/test.sh` flow). **Trap avoided:** lines 1–528 deliberately untouched — line 37's image path is `/docs/assets/images/…` in that repo and a blind copy of GBot's relative path would break the docs site. The Jekyll `docs/` site needed nothing (independent of the README, zero updater/deployment refs) |
| 2c — GPUH shutdown (Pi process + rc.local line) → GitHub repo delete (maintainer, session pauses) → Pi clone delete (very last) | **done 2026-07-26** — GPUH fully retired: service stopped and its port freed, boot-launcher line removed (backed up first; launcher file verified syntactically clean afterwards), GitHub repo deleted by the maintainer, Pi clone removed. One undocumented prerequisite handled first — see below. No collateral: the GBot container and both deploy watchers were unaffected |

**⚠ Lesson: 2a/2b were not sufficient to declare "no remaining consumers."** They cleared the two project repos, but an unrelated host-level utility on the Pi — nothing to do with GBot — also referenced GPUH and would have broken when it disappeared. It was found by grepping the **whole host** rather than just the project repos, and was disabled before GPUH was touched. For any future service retirement, do that host-wide sweep as an explicit pre-flight step; a repo-scoped grep will miss integrations that live in host config or ops tooling.

**Secrets audit (run before the clone was destroyed, since its `.git` was by then the last copy of that history).** Both repos *tracked* their env/key files, but every committed version was an empty template — real values existed only as uncommitted local modifications on the Pi. GBot's own history matches: `Shared/gbot.env` was tracked across 16 commits, all with empty values, untracked for good in `33461ad`; `Shared/serviceAccountKey.json` was never committed at all. **No credential was ever committed to either repo — nothing to rotate.** Before deletion, the surviving copies under `GBot/Shared/` were hash-verified identical to the ones being destroyed.

*(One unrelated hygiene issue was found on the Pi during the host-wide sweep and reported to the maintainer directly rather than recorded here — this file is public.)*
