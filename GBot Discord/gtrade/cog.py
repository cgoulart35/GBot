...
class GTrade(commands.Cog):
    ...
    async def on_guild_remove(self, guild: nextcord.Guild):
        ...
        gtrade.queries.removeAllServerPendingTradeTransaction(guild.id)
    ...
    async def remove_expired_transactions(self):
        ...
        allServersTransactionsMap = gtrade.queries.getAllTradeTransactions()
    ...
    async def craft(self, ctx: Context, name, value, type):
        ...
        try:
            ...
            gcoin.queries.performTransaction(value, date, sender, receiver, 'crafted', '', False, True)
    ...
    async def rename(self, ctx: Context, item, name):
        ...
        try:
            ...
            if itemIdToRename == None:
                ...
                return
            gtrade.queries.renameItem(userId, itemIdToRename, name)
            ...
            gtrade.queries.renameItemRelatedPendingTradeTransactions(userId, item, name)
    ...
    async def destroy(self, ctx: Context, item):
        ...
        if itemTuple != None:
            ...
            gtrade.queries.removeItem(authorId, itemTuple[0])
            # remove any related pending transactions
            gtrade.queries.removeRelatedPendingTradeTransactions(authorId, item)
    ...
    async def market(self, ctx: Context):
        ...
        allServerPending = gtrade.queries.getAllServerPendingTradeTransactions(serverId)
        ...
        try:
            ...
            if itemTuple != None:
                ...
                # create a request to buy from a user if no pending sells exist (market or request)
                    ...
                    pendingBuyTrx = gtrade.queries.getPendingTradeTransaction(serverId, 'buy', item, userId, authorId)
                    # if buy request already created, remove it
                    if pendingBuyTrx != None:
                        gtrade.queries.removePendingTradeTransaction(serverId, pendingBuyTrx[0])
                        await ctx.send(f"{authorMention}, you are no longer requesting to buy '{item}' from {userMention}.")
                    else:
                        existingItems = gtrade.queries.getAllUserItems(authorId)
            ...
            try:
                ...
                if user is None:
                    ...
                    # if pending market sale for item already, remove it
                        ...
                        gtrade.queries.removePendingTradeTransaction(serverId, pendingMarketTrx[0])
                    ...
                    if authorId == userId:
                            raise EnforceSenderReceiverNotEqual
                    pendingBuyTrx = gtrade.queries.getPendingTradeTransaction(serverId, 'buy', item, authorId, userId)
                    pendingSellTrx = gtrade.queries.getPendingTradeTransaction(serverId, 'sell', item, authorId, userId)
                    # if sell request already created, remove it
                    if pendingSellTrx != None:
                        gtrade.queries.removePendingTradeTransaction(serverId, pendingSellTrx[0])
    ...
    def completeTradeTransaction(self, serverId, date, pendingTrxId, itemTuple, seller, buyer):
        ...
        existingItems = gtrade.queries.getAllUserItems(buyer['id'])
        if existingItems != None:
            if len(existingItems) >= self.NUM_MAX_ITEMS:
                raise ItemMaxCount
            for existingItem in existingItems.values():
        ...
        dataType = itemTuple[1]['dataType']
        dataJson = itemTuple[1]['dataJson']
        gtrade.queries.createItem(buyer['id'], originalName, originalValue, originalCreator, originalServer, dateCreated, name, value, date, dataType, dataJson)
        gtrade.queries.removeItem(seller['id'], itemTuple[0])
