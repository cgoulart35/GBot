    ...
    async def post(data):
        try:
            ...
            if "doRebuildLatest" in value and value["doRebuildLatest"] == True:
                ...
                if response == None:
                    return {"action": "doRebuildLatest", "status": "failure", "message": "Error: Can't communicate with the Git Updater API."}
                if "status" not in response:
                    return {"action": "doRebuildLatest", "status": "failure", "message": "Error: Missing status in response from Git Updater API."}
                if "message" not in response:
                    return {"action": "doRebuildLatest", "status": "failure", "message": "Error: Missing message in response from Git Updater API."}
    ...
    async def sendRequestToGitUpdaterHost():
        try:
            async with httpx.AsyncClient() as httpxClient:
                ...
                response = None
                response = await httpxClient.post(url, data = {'application': 'GBot'})
