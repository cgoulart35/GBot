...
1. `plans/active/feature-modernization-roadmap.md` — missed-bug sweep, multi-instance/sharding decision (maintainer discussion first), music + Spotify + whole-bot feature passes, App-Directory north star. **In progress** — the music bug ledger C-1…C-14 is fully closed (C-PR8 shipped 2026-08-02). Next two change sets, in order: **E-PR1** — group all 36 slash + prefix commands under their cogs (`/play` → `/music play`, `.play` → `.music play`), approved 2026-08-04 as Decision E-1 — then **C-PR7**, the queue-UX pass, which lands in the grouped shape.
...
- **`/prod-up`** / **`/prod-down`** / **`/prod-logs [N]`** — start / stop / inspect the prod container (`GBot_8.0_prod`) from the published GHCR image. `/prod-logs` is read-only; on the Pi, container start/stop is maintainer-run.
